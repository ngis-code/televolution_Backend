...
build_middleware(){
    ...
    docker run -d --restart=always --network myNetwork -p 3000:3000 --name televolution_middleware televolution_middleware:$latestMiddlewareReleasedVersion
    if [ $? -ne 0 ]; then
...
load_appwrite_image(){
    ...
    if [[ " ${selected_images[@]} " =~ " televolution_middleware " ]]; then
        ...
        docker run -d --restart=always --network myNetwork -p 3000:3000 --name televolution_middleware televolution_middleware:v0.0.1
        if [ $? -ne 0 ]; then
            error_exit "Failed to start the middleware container."
        fi
